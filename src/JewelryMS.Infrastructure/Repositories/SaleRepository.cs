...
public class SaleRepository : BaseRepository, ISaleRepository
{
    ...
    public async Task<bool> CreateSaleTransactionAsync(
        ...
        IDbTransaction transaction) // Removed optional nullability
    {
        // SOLUTION: Strictly use the transaction's connection
        ...
        if (items != null && items.Any())
        {
            ...
            const string updateInventoryStatusSql = "UPDATE products SET status = 'Sold'::product_status WHERE id = @ProductId";
            
            foreach (var saleItem in items)
            {
                await dbConnection.ExecuteAsync(updateInventoryStatusSql, new { saleItem.ProductId }, transaction);
            }
        }
        ...
        if (exchange != null)
        {
            const string insertExchangeRecordSql = @"
                ...
                VALUES (
                    @Id, @SaleId, @ShopId, @MaterialType::material_type, @Purity::metal_purity, 
    ...
    public async Task<Sale?> GetByInvoiceNoAsync(string invoiceNo, IDbTransaction transaction)
    {
        ...
        const string fetchSaleByInvoiceSql = @"
            SELECT id, shop_id, customer_id, sold_by_id, invoice_no, sale_date, 
                   gross_amount, discount_amount, payment_method::text AS PaymentMethod, 
                   exchange_amount, net_payable, vat_amount, discount_percentage, 
                   status::text AS Status, remarks
            FROM sales 
            WHERE invoice_no = @invoiceNo";

        return await dbConnection.QueryFirstOrDefaultAsync<Sale>(fetchSaleByInvoiceSql, new { invoiceNo }, transaction);
    }

    public async Task UpdateSaleAndInsertItemsAsync(Sale sale, List<SaleItem> items, IDbTransaction transaction)
    {
        var dbConnection = transaction.Connection ?? throw new ArgumentNullException(nameof(transaction));
        ...
        const string finalizeStockStatusSql = "UPDATE products SET status = 'Sold'::stock_status WHERE id = @ProductId";

        foreach (var finalizedItem in items)
        {
            await dbConnection.ExecuteAsync(insertFinalizedItemSql, finalizedItem, transaction);
            await dbConnection.ExecuteAsync(finalizeStockStatusSql, new { finalizedItem.ProductId }, transaction);
        }
    }

    // Reporting methods use internal connections as they are Read-Only and usually outside UoW
    public async Task<IEnumerable<InvoiceDetailResponse>> GetInvoiceDetailsForPdfAsync(string invoiceNo)
    {
        ...
        return await dbConnection.QueryAsync<InvoiceDetailResponse>("SELECT * FROM view_pdf_invoice WHERE invoice_no = @InvoiceNo", new { InvoiceNo = invoiceNo });
    }

    public async Task<IEnumerable<InvoiceDetailResponse>> GetFullAdminReportAsync(string invoiceNo)
    {
        using var dbConnection = await GetOpenConnectionAsync();
        return await dbConnection.QueryAsync<InvoiceDetailResponse>("SELECT * FROM view_full_invoice_report WHERE invoice_no = @InvoiceNo", new { InvoiceNo = invoiceNo });
    }

    public async Task<KachaMemoResponse?> GetKachaMemoDetailsAsync(string invoiceNo)
    {
        using var dbConnection = await GetOpenConnectionAsync();
        return await dbConnection.QueryFirstOrDefaultAsync<KachaMemoResponse>("SELECT * FROM view_kacha_memo WHERE invoice_no = @InvoiceNo", new { InvoiceNo = invoiceNo });
    }
}