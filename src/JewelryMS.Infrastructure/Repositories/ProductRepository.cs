...
public class ProductRepository : BaseRepository, IProductRepository
{
    ...
    // Helper to get connection from transaction or base
    ...
    public async Task<Guid> AddWithAuditAsync(Product product, Guid userId, string role, IDbTransaction? transaction = null)
    {
        ...
        var id = await conn.ExecuteScalarAsync<Guid>(sql, new {
            ...
            product.CostMakingCharge, UserId = userId
        }, transaction);
    ...
    public async Task<bool> UpdateWithAuditAsync(Product product, Product? oldProduct, Guid userId, string role, IDbTransaction? transaction = null)
    {
        ...
        const string sql = @"UPDATE public.products SET 
                              sku=@Sku, name=@Name, sub_name=@SubName, purity=@Purity::metal_purity, 
                              category=@Category::jewelry_category, base_material=@BaseMaterial::material_type, 
                              gross_weight=@GrossWeight, net_weight=@NetWeight, making_charge=@MakingCharge, 
                              cost_metal_rate=@CostMetalRate, cost_making_charge=@CostMakingCharge,
                              status=@Status::stock_status, updated_at=NOW(), updated_by=@UserId 
                              WHERE id=@Id";
        ...
        var affected = await conn.ExecuteAsync(sql, new {
            ...
            product.CostMetalRate, product.CostMakingCharge, product.Status, product.Id, UserId = userId
        }, transaction);
    ...
    {
        var conn = await GetContextConnection(transaction);
        var affected = await conn.ExecuteAsync("DELETE FROM public.products WHERE id = @Id", new { product.Id }, transaction);
    ...
    public async Task<IEnumerable<Product>> GetAllAsync()
    {
        ...
        const string sql = "SELECT *, purity::TEXT, category::TEXT, base_material::TEXT, status::TEXT FROM public.products";
    ...
    {
        var conn = await GetContextConnection(transaction);
        const string sql = "SELECT EXISTS(SELECT 1 FROM public.products WHERE sku = @Sku AND shop_id = @ShopId)";
